...
from .errors import ChainCodecAPIError, ChainCodecTransportError
...
def _default_transport(url: str, headers: Mapping[str, str], timeout: float) -> tuple[int, str]:
    ...
    try:
        ...
        raise ChainCodecTransportError(f"request failed: {exc.reason}") from exc


class ChainCodecClient:
    """Client for the ChainCodec /api/decode endpoint."""
    ...
    def __init__(
        ...
        *,
        ...
        if not base_url or not base_url.strip():
            raise ValueError("ChainCodecClient: base_url is required")
    ...
    def decode_tx(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise ChainCodecTransportError(
                "unexpected response format from ChainCodec API",
...
def _load_json(body: str) -> Any:
    ...
    try:
        ...
        raise ChainCodecTransportError("invalid JSON response from ChainCodec API") from exc
...
def _api_error_from_response(
    ...
    if isinstance(payload, dict):
        ...
        if isinstance(envelope, dict):
            message = str(
                envelope.get("message") or f"ChainCodec API request failed with HTTP {status_code}"
            )
    ...
    fallback = f"ChainCodec API request failed with HTTP {status_code}"
    if body.strip():
